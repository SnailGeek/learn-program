            ...
            try {
                ...
                do {
                    ...
                    try {
                        ...
                        // If `next` or `hasNext` throws (they can, since it is user-provided), we need to treat the stream as errored as per rule 1.4
                        // 如果next方法或hasNext方法抛异常（用户提供的），认为流抛异常了发送onError信号
                        terminateDueTo(t);
                        return;
                    }
